...
function BannerEditPage() {
  ...
  useEffect(() => {
    ...
    getBannerById(bannerId).then((res) => {
      ...
      setImages([]); // Reset images to empty array for new uploads
      setExistingImages(data.images || []); // Store existing images separately
      setRemovedImageIds([]); // Reset removed images to empty array
    });
  }, [bannerId, isEditMode]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        ...
        const finalImages = [
          ...existingImages.filter(img => !removedImageIds.includes(img.imageId)),
          ...(images || [])
        ];
                    ...
                    {existingImages.map((img) => (
                      <div key={img.imageId} className="relative">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            setRemovedImageIds([...removedImageIds, img.imageId]);
                            setExistingImages(
